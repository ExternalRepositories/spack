...
class SpackSolverSetup(object):
    ...
    def validate_variant_value(self, spec, variant_name, value):
        """Validate variant values from input specs."""
        if variant_name not in spack.directives.reserved_names:
            variant_def = spec.package.variants[variant_name]
            variant_def.validate_or_raise(
                spec.variants[variant_name], spec.package)
    ...
    def spec_clauses(self, spec, body=False):
        ...
        for vname, variant in sorted(spec.variants.items()):
            ...
            for value in values:
                ...
                self.validate_variant_value(spec, vname, value)
